...
namespace CraftTech.WebUI.Areas.Admin.Controllers
{
    ...
    public class BannerController : Controller
    {
        ...
        public async Task< IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7117/api/Banner");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7117/api/Banner/{id}");
            if(responseMessage.IsSuccessStatusCode)
            {
                var jsonData= await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateBanner(BannerUpdateDto bannerUpdateDto, IFormFile? file)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7117/api/Banner", content);
            if(responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> AddBanner(BannerCreateDto bannerCreateDto, IFormFile file)
        {
            ...
            StringContent content=new StringContent(jsonData,Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:7117/api/Banner", content);
            if(responseMessage.IsSuccessStatusCode) 
            {
                return RedirectToAction("Index"); 
            }
